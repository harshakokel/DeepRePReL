...
class MpiAdam(Optimizer):
    def __init__(self,
        ...
        if ptu.get_mode() == "gpu_opt":
            ...
            self.m = torch.zeros(total_params, dtype=torch.float32).to(ptu.device)
            self.v = torch.zeros(total_params, dtype=torch.float32).to(ptu.device)
        else:
            print(ptu.get_mode())
    ...
    def step(self, closure=None):
        ...
        if self.comm is not None:
            ...
            globalg = ptu.from_numpy(globalg)
        else:
            globalg = ptu.from_numpy(localg)
